...
implementation {
	...
	*/
	...
	uint8_t findIndexOfMaximumId(am_addr_t arr[]){
		int8_t i;
		uint8_t size = sizeofArray(arr);
	...
	void checkIfBlack(){
		uint8_t maxIdIndex = findIndexOfMaximumId(whiteNodes);

		if(myColor == WHITE){
			
			if(maxIdIndex != EMPTY_INDEX){
			
				dbg("printColor2","*** Mote-(%d) - checkIfBlack(); [%d]: %d, size: %d \n", TOS_NODE_ID, maxIdIndex, whiteNodes[maxIdIndex],sizeofArray(whiteNodes));
				
				if(TOS_NODE_ID > whiteNodes[maxIdIndex]){
					myColor = BLACK;
	...
	event void Boot.booted() {	
		...
		uint8_t i;
		uint8_t size = sizeofArray(topology9[TOS_NODE_ID - 1]);
		
		for(i = 0; i < size; i++){
			neighborNodes[i] = topology9[TOS_NODE_ID - 1][i];
		}
